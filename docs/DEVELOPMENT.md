...
第一条命令会校验全部 VRM 的 GLB 文件头、字节数和 SHA-256；任一素材缺失或被篡改时不会写数据库。脚本不再依赖任何用户 `Documents` 目录。
...
游客感受度自动日报/周报使用独立 worker，开发环境在第三个终端启动：

```powershell
..\scripts\dev-report-worker.ps1
```

worker 每分钟检查一次计划，自动补齐已启用景区的默认计划，并以“景区 + 报告类型 + 统计周期”去重。服务中断后会补建最近应生成的完整周期，并重试最多 3 次；管理端可调整每日/每周执行时间、导出 Word，或使用“打印 / PDF”保存为 PDF。部署环境应将该命令配置为独立常驻进程，不能只启动 Web API。
